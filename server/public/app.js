...
async function homeView() {
  ...
  document.body.classList.remove('session-mode');
  const sessions = await jget('/api/sessions'); // server sorts by last activity, newest first
  const byRoot = {};
  ...
  input.addEventListener('input', () => {
    ...
    if (home.q.length >= 3) {
      ...
      timer = setTimeout(async () => {
        const q = home.q;
        try {
          const hits = await jget(`/api/search?q=${encodeURIComponent(q)}`);
...
async function sessionView(id) {
  ...
  const [manifest, jsonl] = await Promise.all([
    jget(`/api/sessions/${id}`),
    tget(`/api/sessions/${id}/transcript`),
  ]);
  const model = parseTranscript(jsonl);
  const snapshots = [];
  for (const name of manifest.snapshots || []) {
    try { snapshots.push({ name, data: await jget(`/api/sessions/${id}/snapshots/${name}`) }); } catch {}
  }
  ...
  fetch(`/api/filehistory/${id}`).then((r) => (r.ok ? r.json() : null)).then((list) => {
  ...
  await Promise.all(metaFiles.map(async (mf) => {
    try {
      ...
      const [meta, text] = await Promise.all([
        jget(`/api/sessions/${s.id}/files/${mf}`),
        tget(`/api/sessions/${s.id}/files/${jf}`),
      ]);
...
function renderItemPreview(s, pane, pv) {
  ...
  if (pv.kind === 'text') {
    ...
    const vurl = (n) => `/api/filehistory/${s.id}/${encodeURIComponent(n)}`;
    const render = () => {
    ...
    const url = `/api/sessions/${s.id}/files/${encodeURIComponent(pv.path)}`;
